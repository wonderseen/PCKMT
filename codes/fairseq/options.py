...
def get_experimental_generation_parser(interactive=False, default_task="translation"):
    ...
    # add_knn_compact_args(parser)
    if interactive:
        add_interactive_args(parser)
    return parser


def get_experimental_knn_align_parser(interactive=False, default_task="translation"):
    parser = get_parser("Generation", default_task)
    add_dataset_args(parser, gen=True)
    add_distributed_training_args(parser, default_world_size=1)
    add_model_args(parser)
...
def add_datastore_args(parser):
    ...
    group.add_argument("--save-knn-compate-feature", default=False, action='store_true')
...
def add_knn_compact_args(parser):
    group = parser.add_argument_group("kNN Compact Recoder")
    group.add_argument("--not-train-knn-compact-projection", default=False, action='store_true')
    group.add_argument("--test-knn-compact-projection", default=False, action='store_true')
    group.add_argument("--create-knn-compact-projection", default=False, action='store_true')
    group.add_argument('--dimension', type=int, default=1024, help='Size of each key')
    group.add_argument("--svd", default=False, action='store_true')
    group.add_argument("--train-svd", type=bool, default=False, help='If this is a setting to train the svd layer')
    group.add_argument("--convex-svd", default=False, action='store_true', help='If use the convex svd result')
