...
class TranslationTask(LegacyFairseqTask):
    ...
    def forward_and_get_hidden_state_step(self, sample, model):
        ...
        decoder_output, extra = model(src_tokens=sample['net_input']['src_tokens'],
                                      ...
                                      features_only=True,
                                      train_knn_projection=False)
        
        return decoder_output

    def forward_and_get_hidden_state_step_and_prob(self, sample, model):
        decoder_output, prob = model(src_tokens=sample['net_input']['src_tokens'],
                                      src_lengths=sample['net_input']['src_lengths'],
                                      prev_output_tokens=sample['net_input']['prev_output_tokens'],
                                      return_all_hiddens=False,
                                      features_only=False,
                                      features_and_prob=True,
                                      train_knn_projection=False
                                      )
        return decoder_output, prob
