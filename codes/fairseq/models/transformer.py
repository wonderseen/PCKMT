...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def forward(
            ...
            train_knn_projection: bool = False 
    ):
        """
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if self.knn_lambda_type == "trainable" and self.knn_k_type == "trainable":
            ...
            )# dexin wang the softmax is not necessary actually, because knn_probs are not summed up as 1.
    ...
    def forward(
        ...
        """
        # why x.shape[0] = 128 when testing batch_size of src_tokens = 32 ?
        ...
        if self.use_knn_datastore:
            ...
            if self.label_count_as_feature:
                ...
                if False:#self.fcg: # dynamicly activate parameters corresponding to different k
                    fcg_mask = knn_search_result['fcg_mask']
                    network_inputs = torch.cat((
                        knn_dists * (~fcg_mask).to(torch.float),
                        label_counts.float() * (~fcg_mask).to(torch.float)),
                        dim=-1).detach() # [B, S, 2*K]
                else:
                    network_inputs = torch.cat((
                        knn_dists.detach(),
                ...
                if self.knn_use_last_layer_weight:
                    # selected_last_layer_weights = functional.embedding(tgt_index, self.output_projection.weight) # 4-47.1 8-47.1
                    # selected_last_layer_weights = last_hidden[:,:, None, :] * functional.embedding(tgt_index, self.output_projection.weight) # lookup and dot
            ...
            if self.knn_lambda_type == "trainable" and self.knn_k_type == 'trainable':
                ...
                if False:# self.fcg: # if we have not enough retrieval numbers, we only pass forward the parameters based on retrieval
                    network_inputs = network_inputs.view(bsz * seq_len, -1)# [B*S, 2*K]
                    temp_retrieve_layer = self.retrieve_result_to_k_and_lambda[0].weight[None]
                    temp_retrieve_layer = temp_retrieve_layer * (network_inputs[:, None, :] != 0.).to(torch.float)# [B*S, 32, 2*K]

                    temp_output = (temp_retrieve_layer * network_inputs[:, None, :]).sum(-1) + self.retrieve_result_to_k_and_lambda[0].bias[None, None]
                    temp_output = temp_output.squeeze(-1)
                    for layer in self.retrieve_result_to_k_and_lambda[1:]:
                        temp_output = layer(temp_output)
                    net_outputs = temp_output.view(bsz, seq_len, -1)
                else:
                    net_outputs = self.retrieve_result_to_k_and_lambda(network_inputs) # [B, S, K]

                if self.fcg:
                    fcg_mask = knn_search_result['fcg_mask']    
                    k_logit = net_outputs  # [B, S, K]
                    ...
                    knn_lambda = 1. - k_prob[:, :, 0: 1]  # [B, S, 1] ? why 1. - 
                    k_soft_prob = k_prob[:, :, 1:]
