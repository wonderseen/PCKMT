...
def _main(args, output_file):
    ...
    if args.not_train_knn_compact_projection:
        ...
        filenames = utils.split_paths(args.path)
        num_shards = 1
        ...
        for shard_idx in range(num_shards):
            if num_shards == 1:
                filename = orig_filename.replace(".pt", suffix + ".pt")
            else:
            ...
            if shard_idx == 0:
                _model_args = state["args"]
                
                # build model for ensemble
                model = task.build_model(_model_args)
            
            model.load_state_dict(state["model"], strict=strict, args=args)

        model.eval()
        model.decoder.predict_on_total_trainset()
